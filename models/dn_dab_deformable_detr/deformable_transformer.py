...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=128, nhead=8,
        ...
        if two_stage:
            ...
            self.pos_trans = nn.Linear(d_model * 5, d_model * 5)
            self.pos_trans_norm = nn.LayerNorm(d_model * 5)
        else:
            if not self.use_dab:
    ...
    def get_proposal_pos_embed(self, proposals):
        num_pos_feats = 128
        temperature = 20  # sjhong. Paper used 20. Defaults: 10000
        scale = 2 * math.pi
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, attn_mask=None):
        ...
        Input:
            - srcs: List([bs, c, h, w])
            - masks: List([bs, h, w])
            - query_embed: List([bs, nq, d_model + 5])
        """
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        Intermedia:
            - reference_points: [bs, sum(hi*wi), num_label, 2]
        """
        output = src
        # bs, sum(hi*wi), 256
    ...
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, use_dab=False, d_model=256,
        ...
        if use_dab:
            ...
            if self.no_sine_embed:
                self.ref_point_head = MLP(5, d_model, d_model, 3)  # sjhong 4 -> 5
            else:
                self.ref_point_head = MLP(5 * d_model, d_model, d_model, 2)  # sjhong 2 -> 5, 1 -> 2
        self.high_dim_query_update = high_dim_query_update
        if high_dim_query_update:
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes,
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 5:
                if not (src_valid_ratios[0][0][0] == 1.).all():
                    print('src_valid_ratios:\n', src_valid_ratios)  # I think ratios are composed of only 1, sjhong
                    raise Exception('Modify src_valid_ratios')
                # reference_points[:, :, None][:, :, :, :4] * torch.cat([src_valid_ratios, src_valid_ratios], -1)[:, None]
                # reference_points_input = reference_points[:, :, None] \
                #                          * torch.cat([src_valid_ratios, src_valid_ratios], -1)[:, None]  # bs, nq, 5, 5
                bs, nq, n = reference_points.shape
                reference_points_input = reference_points.repeat(1, 1, n).reshape(bs, nq, n, n)
            else:
...
def gen_sineembed_for_position(pos_tensor):
    ...
    dim_t = temperature ** (2 * torch.div(dim_t, 2, rounding_mode='floor') / 128)
    x_embed = pos_tensor[:, :, 0] * scale
    y_embed = pos_tensor[:, :, 1] * scale
    pos_x = x_embed[:, :, None] / dim_t
    pos_y = y_embed[:, :, None] / dim_t
    pos_x = torch.stack((pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=3).flatten(2)  # bs, nq, 128
    pos_y = torch.stack((pos_y[:, :, 0::2].sin(), pos_y[:, :, 1::2].cos()), dim=3).flatten(2)  # bs, nq, 128
    if pos_tensor.size(-1) == 2:
        pos = torch.cat((pos_y, pos_x), dim=2)
    elif pos_tensor.size(-1) == 5:
        w_embed = pos_tensor[:, :, 2] * scale
        pos_w = w_embed[:, :, None] / dim_t
        pos_w = torch.stack((pos_w[:, :, 0::2].sin(), pos_w[:, :, 1::2].cos()), dim=3).flatten(2)  # bs, nq, 128

        h_embed = pos_tensor[:, :, 3] * scale
        pos_h = h_embed[:, :, None] / dim_t
        _tmp = torch.stack((pos_h[:, :, 0::2].sin(), pos_h[:, :, 1::2].cos()), dim=3)
        pos_h = torch.stack((pos_h[:, :, 0::2].sin(), pos_h[:, :, 1::2].cos()), dim=3).flatten(2)  # bs, nq, 128

        a_embed = pos_tensor[:, :, 4] * scale
        pos_a = a_embed[:, :, None] / dim_t
        pos_a = torch.stack((pos_a[:, :, 0::2].sin(), pos_a[:, :, 1::2].cos()), dim=2).flatten(2)  # bs, nq, 128

        pos = torch.cat((pos_y, pos_x, pos_w, pos_h, pos_a), dim=2)  # bs, nq, 640
    else:
